...
class TestBOM:
    def test_bom_json_detected(self):
        bom = "\ufeff" + json.dumps([{"id": i, "name": f"item_{i}", "value": i * 10} for i in range(50)])
        eng = CompressionEngine(min_tokens=100)
        _, meta = eng.process("tool", bom)
        assert meta.get("kind") == "json→csv"

    def test_no_bom_json(self):
        normal = json.dumps([{"id": i, "name": f"item_{i}", "value": i * 10} for i in range(50)])
        eng = CompressionEngine(min_tokens=100)
...
class TestQualityMetrics:
    ...
    def test_anomaly_detection(self):
        # 基礎版不做異常檢測，anomaly_recall 為 0（異常檢測為企業版功能）
