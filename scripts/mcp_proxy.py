# -*- coding: utf-8 -*-
"""mcp_proxy.py — v5.3 MCP 透明代理（攔截式工具回傳壓縮）
...
class CompressionEngine:
    ...
    def _store_full(self, text: str) -> str:
        # v5.3.2 修復：handle 改用內容 sha256（前 12 位）取代遞增計數器。
    ...
    def process(self, tool_name: str, result_text: str) -> tuple:
        ...
        # v5.3.3 BOM 修復：Windows 檔案常帶 \ufeff，會擋住 JSON 判斷
        if stripped[:1] == "\ufeff":
            stripped = stripped[1:]
        ...
        if stripped[:1] in ("[", "{"):
            try:
                ...
                # v5.5：對話格式優先用 conversation_compressor
                ...
                if should_compress_json(data):
                    compressed = compress_json_enhanced(data)  # v5.3.5 增強版：常數欄位+空欄位+巢狀扁平化
                    handle = self._store_full(result_text)
                    saved = len(result_text) - len(compressed)
                    self.stats["compressed"] += 1
                    self.stats["bytes_saved"] += saved
        ...
        # v6.9.3 code-aware 路由（B 任務）：fenced code / code-like → 去重+空白精簡。
        ...
        # v5.5：長文字用關鍵行提取（替代單純 head/tail，數據/錯誤/結論行優先）
...
def self_test():
    ...
    # 長文字壓縮（v5.5：超過50行走 keyline，否則走 head/tail）
