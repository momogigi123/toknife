...
# v5.7 結構化摘要 schema 關鍵字（G-3，對齊 Claude Code Auto-Compact 欄位）
SCHEMA_KEYWORDS = {
...
def extract_state(messages: List[Dict[str, str]], sensitivity: str = "balanced") -> Dict[str, Any]:
    ...
    sensitivity（v5.7 G-4）：high=指令層保留多 / low=上下文層激進壓。
    """
    state = {
        "task_goal": "",
        "completed": [],
    ...
    # v6.7.2 關鍵句保護：偏好/記憶/決策訊號句併入 key_facts
...
def compress_messages(
    ...
    參數：
        ...
        sensitivity: v5.7 G-4，high=指令層保留多 / low=上下文層激進壓
        schema: v5.7 G-3，結構化摘要欄位（如 ("done","blocked","next")）；給定則用 schema 摘要

    返回：
        壓縮後的訊息列表：system 指令原樣保留 + 一條歷史摘要 system + 最近 N 輪原文
    """
    # v5.7 G-4：system 訊息（指令層）永遠原樣保留，不進壓縮
    ...
    # v6.7.2 偏好/關鍵句保護：使用者偏好與決策句附加到摘要，
...
def _extract_pref_sents(messages: List[Dict[str, str]], max_n: int = 12) -> List[str]:
    """v6.7.2：從舊訊息收集使用者偏好/決策句（記住/偏好/喜歡/請務必/規則/決定/採用/確認）。
...
def auto_compact(messages: List[Dict[str, str]], context_limit: int,
    ...
    """v5.7 G-3：當上下文用量佔比 >= threshold 自動觸發壓縮（v6.3 閾值 95%→75%，
...
# ===== v6.3 V-4a：anchored 增量合併壓縮（對齊 Factory Anchored Iterative Summarization）=====
...
def anchored_compact(messages: List[Dict[str, str]], context_limit: int,
                     ...
                     current_tokens: int = None) -> Dict[str, Any]:
    """v6.3：上下文佔比 >= threshold 時，只對「新丟棄區段」（messages[:-keep_recent]）做增量摘要，
