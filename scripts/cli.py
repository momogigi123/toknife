...
def cmd_agg(args):
    ...
    if not isinstance(data, list):
        print("[聚合] 輸入必須是 JSON 陣列", file=sys.stderr)
        return 1
    ...
    formatted = agg.format_for_llm(result, compact=args.compact, max_cell_len=args.max_cell_len)
    print(formatted)

    if args.stats:
        print(f"\n[統計] 原始 {len(data)} 行 → Top-{result.get('current_top_n')} + {len(result.get('anomalies', []))} 異常")
    return 0
...
def main():
    parser = argparse.ArgumentParser(
        prog="ts",
        description="tokknife — 上下文 token 壓縮工具集（，合併豆包版）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__
    )
    parser.add_argument("--version", action="version", version=f"tokknife v{__version__}")
    ...
    # agg
    p_agg = sub.add_parser("agg", help="校準聚合（Top-N + 異常 + 統計）")
    p_agg.add_argument("src", help="JSON 陣列檔案路徑")
    p_agg.add_argument("--sort-by", help="排序字段（預設第一個數值字段）")
    p_agg.add_argument("--top-n", type=int, help="Top-N 行數（預設校準自動）")
    p_agg.add_argument("--compact", action="store_true", help="緊湊格式（長 cell 截斷）")
    p_agg.add_argument("--max-cell-len", type=int, default=24, help="緊湊模式 cell 最大長度")
    p_agg.add_argument("--stats", action="store_true", help="顯示統計資訊")
