# -*- coding: utf-8 -*-
"""
Toknife Lite v3.1 — 增量式上下文壓縮
不是定期壓縮，而是每條新消息進來時增量更新對話狀態。
舊消息可以直接丟棄，狀態向量控制在 200 token 以內。

分工（v5.7 去重合併後）：本模組 =『每條消息流式』維護狀態 + should_compress() 動態閾值守門；
...
def _est_tokens(text: str) -> int:
    """估算 token 數（中文 1.5/字，其他 0.3/字元）；v5.2 提取為 module-level 供 should_compress 用。"""
...
class IncrementalCompressor:
    ...
    def build_context(self, include_state: bool = True, include_recent: bool = True,
        ...
        構建精簡上下文：狀態 + 最近 N 輪。
        可以直接傳給 LLM。
        v5.2：auto_skip=True 時，若 should_compress() 為 False（短對話無省益），
        直接回傳原始消息，避免反增 token。
        """
